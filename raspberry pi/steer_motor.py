...
import sys
import math
import serial    // 라즈베리와 시리얼통신을 할 것이라서 library를 import한다.
                 // serial 통신을 할 때 piserial을 써야한다.
import time

port = "/dev/ttyACM0"     // 아두이노를 연결했을 때 연결된 port를 입력한다. 
serialFromArduino = serial.Serial(port,9600)
serialFromArduino.flushInput()     // 포트접속 초기화
    ...
    while True:
        ...
        if lines is not None:
            ...
            if f != -1:
                ...
                if degree > 130:
                    serialFromArduino.write(b'1')     // 아두이노로 1 신호를 보낸다는 것이다. (1)
